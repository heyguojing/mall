...
CREATE TABLE IF NOT EXISTS `think_node` ( (
  ...
  KEY `status` (`status`),
  KEY `name` (`name`)
) ENGINE=MyISAM AUTO_INCREMENT=0 DEFAULT CHARSET=utf8 COMMENT='节点表'
...
class Rbac {
    ...
    static public function authenticate($map,$model='') {
        if(empty($model)) $model =  config('rbac.USER_AUTH_MODEL');
        //使用给定的Map进行认证
    ...
    static function saveAccessList($authId=null) {
        if(null===$authId)   $authId = session(config('rbac.USER_AUTH_KEY'));
    ...
    static function checkAccess() {
        ...
        if( config('rbac.USER_AUTH_ON') ){
			$_module	=	array();
			$_action	=	array();
            if("" != config('rbac.REQUIRE_AUTH_MODULE')) {
                //需要认证的模块
                $_module['yes'] = explode(',',strtoupper(config('rbac.REQUIRE_AUTH_MODULE')));
            }else {
                //无需认证的模块
                $_module['no'] = explode(',',strtoupper(config('rbac.NOT_AUTH_MODULE')));
            }
            //检查当前模块是否需要认证
    ...
    static public function AccessDecision($appName="") {
        ...
        if(self::checkAccess()) {
            ...
            if(empty(session(config('rbac.ADMIN_AUTH_KEY')))) {
    ...
    static public function getAccessList($authId) {
        // Db方式权限数据
        //$db     =   Db::getInstance(config('rbac.RBAC_DB_DSN'));
        $table = array('role'=>config('rbac.RBAC_ROLE_TABLE'),'user'=>config('rbac.RBAC_USER_TABLE'),'access'=>config('rbac.RBAC_ACCESS_TABLE'),'node'=>config('rbac.RBAC_NODE_TABLE'));
	...
	static public function getModuleAccessList($authId,$module) {
        // Db方式
        //$db     =   Db::getInstance(config('rbac.RBAC_DB_DSN'));
        $table = array('role'=>config('rbac.RBAC_ROLE_TABLE'),'user'=>config('rbac.RBAC_USER_TABLE'),'access'=>config('rbac.RBAC_ACCESS_TABLE'));
