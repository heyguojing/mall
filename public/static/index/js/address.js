...
function edit_address (obj, address_id) {
	var _posturl = "/Address/editAddress";
	$.ajax({
		type: 'POST',
		url: _posturl,
		data: {
		...
		success: function (data) {
			if (data.status == 1) {
				...
				//默认地址
				if (data.address_one.is_default == 1) {
...
function address_form (method, obj) {
	method = method || add;
	if (method == 'add') {
		var _posturl = '/Address/add';
	} else {
		var _posturl = '/Address/edit';
	}
	//用户名
	...
	$.ajax({
		...
		success: function (data) {
			if (data.status == 1) {
				var _html = "";
				if (method == "edit") {
					$("[data-address_id=" + data.address_id + "]").attr('data-address_province', data.province_name);
					$("[data-address_id=" + data.address_id + "]").attr('data-address_city', data.city_name);
					$("[data-address_id=" + data.address_id + "]").attr('data-address_district', data.district_name);
					$("[data-address_id=" + data.address_id + "]").attr('data-address_username', data.address_username);
					$("[data-address_id=" + data.address_id + "]").attr('data-user_address', data.user_address);
					$("[data-address_id=" + data.address_id + "]").attr('data-address_mobile', data.address_mobile);
					$("[data-address_id=" + data.address_id + "]").attr('data-is_default', data.is_default);
					var li = $("[data-address_id=" + data.address_id + "]").next();
					li.find(".buy-user").html(data.address_username);
					li.find(".buy-phone").html(data.address_mobile);
					li.find(".province").html(data.province_name);
					li.find(".city").html(data.city_name);
					li.find(".district").html(data.district_name);
					li.find(".street").html(data.user_address);
					set_post_address(data);
					//设置默认之后 其他地址去除默认标识
					if (data.is_default == 1) {
						...
						li.find(".del_address").attr('onclick','del_address(this,'+data.address_id+',1)');
					}
					$(document.body).css("overflow", "visible");
					$('.theme-login').removeClass("selected");
					$('.item-props-can').removeClass("selected");
					...
					_html += '<div class="per-border" data-address_id="' + data.address_id + '" data-address_province="' + data.province_name + '" data-address_city="' + data.city_name + '"  data-address_district="' + data.district_name + '" data-user_address="' + data.user_address + '" data-address_username="' + data.address_username + '" data-address_mobile="' + data.address_mobile + '" data-is_default="' + data.is_default + '"></div>';
					if (data.is_default == 1) {
					...
					_html += '<span class="buy-user">' + data.address_username + '</span>';
					_html += '<span class="buy-phone">' + data.address_mobile + '</span>';
					_html += '</span>';
					_html += '		</div>';
					if (data.is_default == 1) {
					...
					_html += '			<span class="buy--address-detail">';
					_html += '				<span class="province">' + data.province_name + '</span>';
					_html += '<span class="city">' + data.city_name + '</span>';
					_html += '<span class="dist">' + data.district_name + '</span>';
					_html += '<span class="street">' + data.user_address + '</span>';
					_html += '			</span>';
					_html += '		</div>';
					if (data.is_default == 1) {
					...
					if (data.is_default == 1) {
						_html += '		<a href="javascript:;" class="is_default  hidden" onclick="set_default_address(this,' + data.address_id + ')">设为默认</a>';
					} else {
						_html += '			<a href="javascript:;" class="is_default" onclick="set_default_address(this,' + data.address_id + ')">设为默认</a>';
					}
					_html += '		<span class="new-addr-bar hidden">|</span>';
					_html += '		<a href="javascript:;" onclick="edit_address(this,' + data.address_id + ');">编辑</a>';
					_html += '		<span class="new-addr-bar">|</span>';
					_html += '		<a href="javascript:void(0);" onclick="del_address(this,' + data.address_id + ',' + data.is_default + ');">删除</a>';
					_html += '	</div>';
					_html += '</li>';
					set_post_address(data);
					//设置默认之后 其他地址去除默认标识
...
function set_post_address (data) {
	$(".pay-address .buy--address-detail .province").html(data.province_name);
	$(".pay-address .buy--address-detail .city").html(data.city_name);
	$(".pay-address .buy--address-detail .dist").html(data.district_name);
	$(".pay-address .buy--address-detail .street").html(data.user_address);
	$(".pay-address .buy-footer-address .buy-user").html(data.address_username);
	$(".pay-address .buy-footer-address .buy-phone").html(data.address_mobile);
