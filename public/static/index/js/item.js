...
function goods_buy (obj, goods_id) {
	...
	//规格id
	var standard_id = spec_compose[had_spec] ? spec_compose[had_spec]['standard_id'] : 0;
	//商品数量小于1
	if (goods_num < 1) {
		layer.msg('至少需要购买一件商品', {icon: 2});
		$("[name=goods_num]").focus();
	...
	$.ajax({
		...
		success: function (data) {
			if (data.status == 0) {
				...
				location.href = "/Order/buy/goods_id/" + goods_id + "/standard_id/" + standard_id + "/goods_num/" + goods_num;
			}
		}
	})
}
