...
$(function () {
	...
	$(".spec").live('click', function () {
		...
		$.post('/goods/getProduct', {goods_id: goods_id}, function (data) {
			if (data.status == 1) {
				$(".select-pop").html('已选择规格:' + "<i>" + spec.spec_value.join("+") + "</i>");
				$(".select-pop-data").html('已选择规格:' + "<i>" + spec.spec_value.join("+") + "</i>");
				$.each(data.standard_data, function (k, v) {
					spec_compose[v.standard_name] = v;
					spec_compose['compose'][v.standard_name] = v.standard_name.split(",");
				});
						...
						$('input', $(this)).each(function () {
							if ($.inArray($(this).val(), allowd) == -1) {
								...
								if (spec_compose[has_spec]) {
									//库存为0直接禁用
									if (spec_compose[has_spec]['standard_stock'] == 0) {
				...
				if (spec_compose[had_spec]) {
					//库存
					$(".stock").text(spec_compose[had_spec]['standard_stock']);
					//价格
					$(".sys_item_price,.price-now").text(spec_compose[had_spec]['standard_price']);
					//划线价
					$(".sys_item_mktprice").text(spec_compose[had_spec]['standard_del_price']);

					if (spec_compose[had_spec]['standard_stock'] == 0) {
					...
					if (_val >= spec_compose[had_spec]['standard_stock']) {
						obj.parents(".theme-signin-left").find("[name=goods_num]").val(spec_compose[had_spec]['standard_stock']);
					...
					$(".tb-btn-buy,.tb-btn-basket").removeClass("disabled");
					$('.stock').text($('.stock').attr('goods_stock'));
					$('.sys_item_price,.price-now').text($('.sys_item_price').attr('goods_price'));
					$('.sys_item_mktprice').text($('.sys_item_mktprice').attr('goods_del_price'));
				}
			}
		}, 'json');
	});
